...
LatestGameState = {
    Players = {},
    Obstacles = {}  -- Stores the positions of obstacles
  }
  ...
  function inRange(x1, y1, x2, y2, range)
    ...
    for _, obstacle in ipairs(LatestGameState.Obstacles) do
      if (obstacle.x == x1 and obstacle.y == y1) or (obstacle.x == x2 and obstacle.y == y2) then
        return false
      end
    end
  ...
  function decideNextAction()
    ...
    if player.energy > 5 and targetInRange then
      ...
      if directions[randomIndex] == "Up" then
        newX, newY = player.x, player.y - 1
      elseif directions[randomIndex] == "Down" then
        newX, newY = player.x, player.y + 1
      elseif directions[randomIndex] == "Left" then
        newX, newY = player.x - 1, player.y
      elseif directions[randomIndex] == "Right" then
        newX, newY = player.x + 1, player.y
      end
      ...
      local blocked = false
      for _, obstacle in ipairs(LatestGameState.Obstacles) do
        if obstacle.x == newX and obstacle.y == newY then
          blocked = true
          break
        end
      end
      ...
      if blocked then
        print(colors.gray .. "Move blocked by obstacle. Attempting to remove obstacle." .. colors.reset)
        if player.energy > 2 then
          ao.send({Target = Game, Action = "RemoveObstacle", Player = ao.id, x = newX, y = newY})
          player.energy = player.energy - 2
        else
          print(colors.red .. "Insufficient energy to remove obstacle." .. colors.reset)
        end
      else
  ...
  -- Handler to remove obstacles.
  Handlers.add(
    "RemoveObstacle",
    Handlers.utils.hasMatchingTag("Action", "RemoveObstacle"),
    function (msg)
      local x = msg.x
      local y = msg.y
      for i, obstacle in ipairs(LatestGameState.Obstacles) do
        if obstacle.x == x and obstacle.y == y then
          table.remove(LatestGameState.Obstacles, i)
          print(colors.green .. "Obstacle removed at (" .. x .. "," .. y .. ")." .. colors.reset)
          break
        end
      end
    end
